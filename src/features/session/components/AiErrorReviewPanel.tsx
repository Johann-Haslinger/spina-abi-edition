...
export function AiErrorReviewPanel(props: { attemptId: string | null; onClose: () => void }) {
  ...
  return (
    <aside className="fixed h-2/3 overflow-y-auto right-6 bottom-6 z-9999 w-120 max-w-[calc(100vw-2rem)] rounded-4xl border border-white/10 bg-slate-950/90 p-5 text-white shadow-2xl backdrop-blur">
      ...
      <section className={`mt-4 rounded-3xl border p-4 ${resultTone}`}>
        <div className="flex items-center justify-between gap-4">
          <div className="text-sm font-semibold">{resultLabel}</div>
          <div className="text-sm font-medium">{Math.round(review.score * 100)}%</div>
        </div>
      </section>

      {review.messageToUser ? (
        <p className="mt-4 text-sm text-white/75">{review.messageToUser}</p>
      ) : null}

      {review.errorExplanation ? (
        <section className="mt-5 rounded-3xl border border-rose-500/15 bg-rose-950/40 p-4">
      ...
      {review.solutionExplanation ? (
        <section className="mt-4 rounded-3xl border border-emerald-500/15 bg-emerald-950/30 p-4">
      ...
      {review.notes ? (
        <section className="mt-4 rounded-3xl border border-white/10 bg-white/5 p-4">
          <div className="text-sm font-semibold text-white">Notizen</div>
